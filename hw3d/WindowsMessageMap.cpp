...
std::wstring WindowsMessageMap::operator()( DWORD msg, WPARAM wp, LPARAM lp ) const noexcept
{
	...
	const auto itr = map.find( msg );

	std::ostringstream oss;
	if( itr != map.end( ) )
	{
		oss << std::left << std::setw( firstColWidth ) << itr->second << std::right;
	}
	else
	{
		std::ostringstream padding;
		padding << "Unknown mesage: 0x" << std::hex << msg;
		oss << std::left << std::setw( firstColWidth ) << padding.str( ) << std::right;
	}
